...
def Calculation_Lambda(parameter_list,x,args):
    #print parameter_list[x],x
    if(parameter_list[x]>args[2] or args[2]<2):
        return 0
    else:
...
array_log10_Lambda=np.array([2.5,2.4])

p1_dist=pm.Uniform("p1", 1, 50)
...
for i in range(3):
    plt.subplot(3,1,i+1)
    plt.plot(traces[sample_i][i], label="p_1", lw=lw)

sample_i=0
for i in range(3):
    plt.subplot(3,1,i+1)
    plt.hist(traces[sample_i][i], bins=30,
              histtype="stepfilled")
